...
void txBad(const char *correctStr) {
	...
	// new baud rate is whichever is higher:
	// *current baud rate* - 25
	// OR 90% of *current baud rate*
	uint_fast16_t br1 = opts.baudRate - 25;
	uint_fast16_t br2 = (((double)opts.baudRate) * ((double)0.90));
	if (br1 > br2) {
		opts.baudRate = br1;
	}
	else {
...
bool checkResend(void (*callback)()) {
	readBuf[4095] = '\0'; // null terminate just in case
	if (strstr(readBuf, "RESEND") != 0) {// contains RESEND
		// resend
		printf("Server asked for resend, decreasing baud rate, increasing parity and sending again...\r\n");
		uint_fast16_t br1 = opts.baudRate - 25;
		uint_fast16_t br2 = (((double)opts.baudRate) * ((double)0.90));
		if (br1 > br2) {
			opts.baudRate = br1;
