...
void txSV_HSTART() {
	...
	for (uint8_t i = 0; i != 5; i++) {
		...
		puts("Sent SV_HSTART!");
		minimodem(readBuf, strlen(allGoodStr), MODE_RECEIVE, opts.baudRate, handshakeConfidence);
		...
		minimodem(resendStr, 256, MODE_TRANSMIT, opts.baudRate, handshakeConfidence);
	}
	if (!sentSV_HSTART) {
