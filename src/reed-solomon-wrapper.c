...
ecc_t eccCreate(uint8_t parityPercent, size_t dataSize) {
	ecc_t ecc = malloc(sizeof(__ecc_t));
	// calculate some stuff
	ecc->parityBytes = (uint16_t)ceil(dataSize * ((double)(parityPercent / ((double)(100)))));
...
size_t eccEncode(ecc_t ecc, void *data, size_t dataSize, uint8_t **encodedDataPtr) {
	...
	encode_rs_char(ecc->rsPtr, data, encoded + dataSize);
	...
	*encodedDataPtr = encoded;
	printf("Encoded payload of size %s%lu%s bytes at address %s%p%s\r\n", B_CYAN, dataSize + ecc->parityBytes, RESET, B_YEL, encoded, RESET);
	return dataSize + ecc->parityBytes;
}
void eccDestroy(ecc_t ecc) {
	// get rid of the structure
	free_rs_char(ecc->rsPtr);
	free(ecc);
}
size_t eccDecode(ecc_t ecc, uint8_t *encodedData, size_t dataSize, uint8_t **decodedDataPtr) {
	...
	// decode the data
	size_t size = decode_rs_char(ecc->rsPtr, encodedData, NULL, 0);
